...
public class StatusActivity extends BaseMinorClass {
    ...
    private class mAdapter extends mCom.comAdapter<ItemData>{
        //父类中有的参数为 private T[] mItemList 和 private int itemLayoutId; 也可以用
        ...
        public void setOnCreateViewHolder(final mCom.ViewHolder holder, View v) {
            ...
            v.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    //ItemData item = itemList[position]; //itemList是活动的全局变量，避免被多个adapter覆盖变化
...
//##############################################################
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //设置底部导航条的动作
        new SetNavigatorClass().setClickOnNavigator(this, 3);  //this指代最近的类的实例, 3用以标记导航栏所处位置

        //填充内部功能条
        //数据输入全部 点击后的功能列表所需的数据
        //先生成数据列表，并保存
        Resources res = getResources();
        String[] func_names = res.getStringArray(R.array.report_func_names); //直接调用
        TypedArray imageAr = res.obtainTypedArray(R.array.report_func_images); //只是获取图片的xml引用方式， 这个用过后需要释放
        String[] funcAr = res.getStringArray(R.array.com_detail_func_types); //功能列表
        int[] indexes = res.getIntArray(R.array.report_func_indexes);  //功能列表选择
        ...
        //调用适配器加入布局中
        RecyclerView recyclerView = (RecyclerView) findViewById(R.id.report_list_RecyclerView);
